...
static ubyte w_wdl_matrix[256][8] = {
...
static ubyte wdl_to_dtz_c[8];

static ubyte wdl_to_dtz[8][256];
static ubyte dtz_to_opp[14][256];
static ubyte dtz_matrix[256][256];

static ubyte w_matrix[16][16];

static char dtz_to_wdl[256];
//static ubyte pawn_to_dtz[6];
...
#if 0
  pawn_to_dtz[0] = DTZ_ILLEGAL;
  pawn_to_dtz[5] = DTZ_MATE;
  if (opp) {
    pawn_to_dtz[3] = opp_draw;
    if (ply_accurate_loss) {
      pawn_to_dtz[1] = DTZ_PAWN_WIN;
      pawn_to_dtz[2] = opp_capt_cwin;
    } else {
      pawn_to_dtz[1] = DTZ_CAPT_WIN;
      pawn_to_dtz[2] = opp_capt_cwin;
    }
    if (ply_accurate_win)
      pawn_to_dtz[4] = opp_capt_closs;
    else
      pawn_to_dtz[4] = opp_capt_closs;
  } else {
    pawn_to_dtz[3] = dtz_draw;
    if (ply_accurate_win) {
      pawn_to_dtz[1] = DTZ_PAWN_WIN;
      pawn_to_dtz[2] = dtz_capt_cwin;
    } else {
      pawn_to_dtz[1] = DTZ_CAPT_WIN;
      pawn_to_dtz[2] = dtz_capt_cwin;
    }
    if (ply_accurate_loss)
      pawn_to_dtz[4] = dtz_capt_closs;
    else
      pawn_to_dtz[4] = dtz_capt_closs;
  }
#endif
...
#if 0
  pawn_to_dtz[0] = W_ILLEGAL;
  pawn_to_dtz[1] = W_WIN;
  pawn_to_dtz[2] = W_CWIN;
  pawn_to_dtz[3] = W_DRAW;
  pawn_to_dtz[4] = W_CLOSS;
  pawn_to_dtz[5] = W_LOSS;
#endif
}
...
int check_mate_pawns_w(long64 idx0, ubyte *table, bitboard occ, int *p)
{
  int i;
  int sq;
  long64 idx, idx2;
...
static int has_moves_pieces(int *pcs, long64 idx0, ubyte *table, bitboard occ, int *p)
{
  int sq;
  long64 idx, idx2;
  bitboard bb;
...
int has_moves_pawns_w(long64 idx0, bitboard occ, int *p)
{
  int i;
  int sq;
  long64 idx, idx2;
...
int has_moves_pawns_b(long64 idx0, bitboard occ, int *p)
{
  int i;
  int sq;
  long64 idx, idx2;
...
static int compute_pieces(int *pcs, long64 idx0, ubyte *table, bitboard occ, int *p)
{
  int sq;
  long64 idx, idx2;
  bitboard bb;
  ...
  for (idx = begin + thread->begin; idx < end; idx++) {
	...
	if (sq < 0x10 && !(bit[sq + 0x10] & occ)) {
	  ...
	  while (bits) {
	    ...
	    for (k = 0; p[k] != sq2; k++);
	    if (pt[k] == BPAWN) {
	      v0 = eval_ep(i, k, sq2, p[i] + 0x08, black_king, 8, 1, occ, p);
	      if (v0 < v1) v1 = v0;
	    }
	    ClearFirst(bits);
	  }
  ...
  for (idx = begin + thread->begin; idx < end; idx++) {
	...
	if (sq >= 0x30 && !(bit[sq - 0x10] & occ)) {
	  ...
	  while (bits) {
	    ...
	    for (k = 0; p[k] != sq2; k++);
	    if (pt[k] == WPAWN) {
	      v0 = eval_ep(i, k, sq2, p[i] - 0x08, white_king, 0, 0, occ, p);
	      if (v0 < v1) v1 = v0;
	    }
	    ClearFirst(bits);
	  }
